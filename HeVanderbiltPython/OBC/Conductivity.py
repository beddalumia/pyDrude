def Conductivity(N,L,eigenEs,eigenCs,cutoff):
    ...
    pylab.plot(w,Rg,'--', c='green')
    pylab.plot(w,Dg,'-', c='red')
    pylab.plot(w,Rg-Dg,'-', c='orange')
    #pylab.plot(w,Ig)
    
    ## f-sum on Re(g) ##
    
    from numBraKet import numIntegration as integrate
    fsum = integrate(Rg,0, 40, w)/(pi/2)
    print('f.sum: ', fsum)
    ...
    from numBraKet import numIntegration as integrate
    Dsum = integrate(Dg,0, 50, w)/(pi/2)
    print('OBCs Dw/pi [from Re(g_D)]: ', Dsum)
