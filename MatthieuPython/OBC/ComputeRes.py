def ComputeRes(N,L,eigenEs,eigenCs,cutoff):
    ...
    while eigenEs[n-1] <= EF: # "each occupied state..."
        ...
        while dw < cutoff: #"...can be excited to a non-occupied state within the cutoff"
            ...
            if eigenEs[m-1] > EF:
                ...
                pylab.scatter(L,w, c = color[m-n-1], edgecolors='black')#, alpha = (1 - (nmax-n)/(nmax/5)))
    ...
    pylab.scatter(L,Fsum, c='green', marker='s')
    pylab.scatter(L,Dsum, c='red', marker='o')
    print('Normalized Dw: ', Dsum)
    print('f-sum: ', Rsum+Dsum)
    
    pylab.figure('Pole Frequencies')
    pylab.scatter(L,cutoff, c = 'gray', edgecolors='none', marker = 'X', alpha = 0.3)
    pylab.scatter(L,gap, c = 'gray', edgecolors='none', marker = 'X', alpha = 0.3)
